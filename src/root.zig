...
pub const AtomicGenerator = struct {
    ...
    last: std.atomic.Value(u128),
    
    pub fn init() AtomicGenerator {
        return .{ .last = std.atomic.Value(u128).init(0) };
    }
...
test "ULID lexicographic ordering" {
    ...
    std.time.sleep(2_000_000); // 2ms to ensure different timestamp
...
test "Ulid format implementation" {
    const ulid = new();
    const str = try std.fmt.allocPrint(std.testing.allocator, "{}", .{ulid});
